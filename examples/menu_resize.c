...
#define BOX_PADS 2 // South padding of the box
...
int menu(List_str title, List_str set) {
    ...
    while(!do_return) {
        do_prints = FALSE;
        // Prints
        wborder(menu_win, 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'); // Box window ***
        for(i = 0; i < title.len; i++) { // Print title
            wctrchprint(menu_win, TITLE_ROW+i, COLS1, " %s ", title.list[i]);
        }
        for(i = 0; i < set.len; i++) { // Print options *** with first letter underlined
            mvwchprint(menu_win, POSITION_Y(i, set.len, title.len), BOX_PADX, COLS1, "%s", set.list[i]);
        }

        // Loop to get pressed key
        while(!(do_prints || do_return)) {
            // Update highlighted & non-highlighted option
            mvwchprint(menu_win, POSITION_Y(old_hl, set.len, title.len), BOX_PADX, COLS1, "%s%*s", set.list[old_hl], HL_PADX, "");
            mvwchprint(menu_win, POSITION_Y(hl, set.len, title.len), BOX_PADX, COLS1, "%*s", HL_PADX, ""); // Print hl padding
            mvwchprint(menu_win, POSITION_Y(hl, set.len, title.len), BOX_PADX+HL_PADX, A_STANDOUT | COLS2, "%s", set.list[hl]);
