...
Game_t play_manche(int score, int n_lifes, bool* holes_occupied) {
    // Init vars
    bool occupied_plant;
    int id, thread_params[LIM_N_ENTITIES][N_ENTITY_PARAMS], stream_speed[N_WATER_STREAM], plants_x[N_PLANTS]; // Matrix to store params for each entity; each line will be passed to thread as params
            ...
            // PLANT
                ...
                if(msg.sig == PLANT_SPAWN_SIG) { // Spawn
                    ...
                    if(gamevar.frog.y == LINE_BANK_1 && msg.x + PLANT_DIM_X > gamevar.frog.x && msg.x < gamevar.frog.x + FROG_DIM_X) {
                        ...
                        gamevar.plants[entity_id].x = thread_params[msg.id][PLANT_X_INDEX];
                ...
                if(msg.y >= LINE_RIVER) {
                    ...
                    for(int i = 0; i < N_PLANTS; i++) {
                        if(gamevar.plants[i].y >= 0 && msg.x + BULLET_DIM_X > gamevar.plants[i].x && msg.x < gamevar.plants[i].x + PLANT_DIM_X) {
                            ...
                            gamevar.plants[i].x = thread_params[id][PLANT_X_INDEX];
                            async_exec(&thread_tids, id, &plant_thread, thread_params[id]);
                            break;
                        }
                    }
