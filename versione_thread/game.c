...
void game(void) {
    ...
    int i, x = game_scr->_maxx/2, y = game_scr->_maxy/2, c, point_x[N_POINT], point_y[N_POINT], points = 0;  // Inizio al centro
    mvwaddch(game_scr, y, x, USR);
    wctrprintw(top_bar, TOP_BAR_ROWS/2, "Points: %d", points);
    wrefresh(top_bar);
    for(i = 0; i < N_POINT; i++) {
