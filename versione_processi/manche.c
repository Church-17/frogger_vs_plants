...
Game_t play_manche(int score, int n_lifes, bool* holes_occupied) {
    ...
    for(int i = 0; i < N_WATER_STREAM; i++) {
        ...
        async_exec(pipe_fds, &process_pids, process_params[id][CROCCODILE_ID_INDEX], &croccodile_process, process_params[id]);
    }
    for(int i = 0; i < N_PLANTS; i++) {
        id = MIN_PLANT_ID + i;
        process_params[id][PLANT_ID_INDEX] = MIN_PLANT_ID + i;
        ...
        async_exec(pipe_fds, &process_pids, process_params[id][PLANT_ID_INDEX], &plant_process, process_params[id]);
    }
            ...
            case FROG_ID:
                if(msg.sig == FROG_SHOT_SIG) { // If frog shot a bullet
                    if(gamevar.frog_bullets[next_frog_bullet].y == FREE_ENTITY) { // If a frog bullet is free...
                        ...
                        process_params[id][BULLET_ID_INDEX] = next_frog_bullet + MIN_FROG_BULLET_ID;
                        process_params[id][BULLET_Y_INDEX] = gamevar.frog.y - 1;
                        process_params[id][BULLET_X_INDEX] = gamevar.frog.x + FROG_DIM_X/2;
                        // Fork
                        async_exec(pipe_fds, &process_pids, process_params[id][BULLET_ID_INDEX], &bullet_process, process_params[id]);
            ...
            if(msg.id >= MIN_CROCCODILE_ID && msg.id < MIN_PLANT_ID) {
                ...
                if(stream_last[entity_stream] == entity_id && (stream_speed[entity_stream] > 0 ? (msg.x > 0) : (msg.x < MAIN_COLS - CROCCODILE_DIM_X))) {
                    ...
                    if(gamevar.croccodiles[entity_stream][next_croccodile_id].y == FREE_ENTITY) { // Check if next croccodile in same stream is free
                        ...
                        process_params[id][CROCCODILE_ID_INDEX] = next_croccodile_id + entity_stream*MAX_CROCCODILE_PER_STREAM + MIN_CROCCODILE_ID;
                        ...
                        async_exec(pipe_fds, &process_pids, process_params[id][CROCCODILE_ID_INDEX], &croccodile_process, process_params[id]); // Calls the fork for croccodile process handling the errors
            ...
            // PLANT
                ...
                if(msg.sig == PLANT_SPAWN_SIG) { // Spawn
                    ...
                    if(gamevar.frog.y == LINE_BANK_1 && msg.x + PLANT_DIM_X > gamevar.frog.x && msg.x < gamevar.frog.x + FROG_DIM_X) {
                        ...
                        async_exec(pipe_fds, &process_pids, process_params[msg.id][PLANT_ID_INDEX], &plant_process, process_params[msg.id]);
                        break;
                    }
                    ...
                    if(gamevar.plants_bullets[entity_id][next_plant_bullet[entity_id]].y == FREE_ENTITY) {
                        ...
                        process_params[id][BULLET_ID_INDEX] = next_plant_bullet[entity_id] + MAX_BULLETS_PER_PLANT*entity_id + MIN_PLANT_BULLET_ID;
                        ...
                        async_exec(pipe_fds, &process_pids, process_params[id][BULLET_ID_INDEX], &bullet_process, process_params[id]); // Fork
